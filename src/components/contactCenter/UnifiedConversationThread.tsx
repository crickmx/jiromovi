  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-950 relative">
          ...
          <>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {autoMode && (
                  <span className="flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-medium bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 rounded-full">
                    <Bot className="w-2.5 h-2.5" /> IA activa
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {isMoviChannel && (
                <button
                  ...
                  title={autoMode ? 'Desactivar modo IA' : 'Modo IA / Automatico'}
                  className={cn('p-1.5 rounded-lg transition-colors', autoMode ? 'bg-violet-100 dark:bg-violet-900/30 text-violet-600' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400')}
                >
      ...
      {autoMode && autoSession && (
        <div className="flex-shrink-0 px-4 py-2 bg-violet-50 dark:bg-violet-900/15 border-b border-violet-100 dark:border-violet-800/30">
          <div className="flex items-center gap-2">
            <Bot className="w-4 h-4 text-violet-600 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-violet-800 dark:text-violet-300">{autoSession.assistant_name}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <div className="flex-1 h-1 bg-violet-100 dark:bg-violet-800/30 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-violet-500 rounded-full transition-all"
                    style={{ width: `${autoSession.total_fields > 0 ? (autoSession.captured_count / autoSession.total_fields) * 100 : 0}%` }}
                  />
                </div>
                <span className="text-[10px] text-violet-600 dark:text-violet-400 flex-shrink-0">
                  {autoSession.captured_count}/{autoSession.total_fields} campos
                </span>
              </div>
            </div>
            <button onClick={stopAutoMode} className="text-[10px] text-violet-500 hover:text-violet-700 font-medium flex-shrink-0">
              Cancelar
            </button>
          </div>
        </div>
      ...
      <div className="flex-shrink-0 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700 px-3 py-2.5 relative">
        ...
        <div className="flex items-center gap-1 mb-2">
          ...
          {isMoviChannel && (
            <button
              onClick={autoMode ? stopAutoMode : openAssistants}
              className={cn('flex items-center gap-1 px-2 py-1.5 rounded-lg transition-colors text-[11px] font-medium ml-auto', autoMode ? 'bg-violet-100 dark:bg-violet-900/30 text-violet-600' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400 hover:text-neutral-600')}
              title="Modo IA"
            >
              <Bot className="w-3.5 h-3.5" /> <span className="hidden sm:inline">{autoMode ? 'IA activa' : 'Modo IA'}</span>
            </button>
          )}
        </div>
