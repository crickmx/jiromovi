...
import {
  ...
  formatConversationName, formatTime, CHANNEL_COLORS, CHANNEL_LABELS,
} from '@/lib/contactCenterTypes';
import { ChannelBadge } from './ChannelBadge';

interface ConversationListProps {
  conversations: CCConversation[];
  selectedId: string | null;
  onSelect: (conv: CCConversation) => void;
  search: string;
...
const CHANNELS: (CCChannel | 'all')[] = ['all', 'wa_movi', 'wa_personal', 'chat', 'seguwallet', 'web_form'];
...
function Avatar({ name, avatarUrl, channel, unread }: { name: string; avatarUrl?: string | null; channel: CCChannel; unread: number }) {
...
export function ConversationList({
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(c =>
        formatConversationName(c).toLowerCase().includes(q) ||
    ...
    return list;
  }, [conversations, filterChannel, filterStatus, search]);
    ...
    <div className="flex flex-col h-full bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-700">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-neutral-100 dark:border-neutral-800">
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-hide">
          {CHANNELS.map(ch => {
            ...
            const label = ch === 'all' ? 'Todos' : CHANNEL_LABELS[ch];
            return (
              <button
                key={ch}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          filtered.map(conv => {
            const name = formatConversationName(conv);
            ...
            return (
              ...
              >
                <Avatar name={name} avatarUrl={conv.avatar_url} channel={conv.channel} unread={conv.unread_count} />
