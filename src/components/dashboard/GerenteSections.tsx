...
function estatusChip(label?: string | null) {
  ...
  if (lc.includes('complet') || lc.includes('termin'))
    return <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400">{label}</span>;
...
// avatar initials color by index
const AVATAR_GRADS = [
...
function EjecutivosPanel({ usuario }: { usuario: Usuario }) {
  const [ejecutivos, setEjecutivos] = useState<EjecutivoItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const items: EjecutivoItem[] = users.map(u => {
        ...
        return { id: u.id, nombre: u.nombre, apellidos: u.apellidos, total: mine.length, urgentes };
      });
    ...
    void load();
    return () => { active = false; };
  }, [usuario.id, usuario.oficina_id]);

  return (
    <Panel
      stripe="bg-gradient-to-r from-[#065F46] to-[#10B981]"
      eyebrow="Mi equipo"
      title="Ejecutivos"
      desc="Trámites activos por ejecutivo"
    >
      {loading ? (
        ...
        <>
          <div className="flex flex-col">
            {ejecutivos.map((e, i) => (
              <div key={e.id} className="flex items-center gap-3 py-2.5 border-b last:border-0 border-neutral-100 dark:border-white/6">
                <div
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  {e.urgentes > 0 ? (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400">
                      {e.urgentes} urgente{e.urgentes !== 1 ? 's' : ''}
                    </span>
                  ) : (
                  ...
                  <div className="w-14 h-[3px] bg-neutral-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        background: AVATAR_GRADS[i % AVATAR_GRADS.length].replace('linear-gradient(135deg,', '').replace(')', '').split(',')[0],
                        width: `${Math.min(100, (e.total / Math.max(...ejecutivos.map(x => x.total), 1)) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="bg-neutral-50 dark:bg-white/5 rounded-xl p-3 mt-1">
            ...
            <div className="flex justify-between">
              ...
              <div className="text-center">
                ...
                <p className="text-[9px] text-neutral-400 dark:text-white/35 mt-0.5">Ejecutivos</p>
              </div>
            </div>
          </div>
        </>
...
function MetasPanel() {
  ...
  return (
    <Panel
      stripe="bg-gradient-to-r from-[#0284C7] to-[#38BDF8]"
      eyebrow="Convención · oficina"
      title="Metas"
      desc="Progreso del equipo hacia convención"
    >
      {data === 'loading' ? (
        <><Sk className="h-16" /><Sk className="h-12" /><Sk className="h-12" /><Sk className="h-12" /></>
      ) : (
        <>
          ...
          {prod && (
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-neutral-50 dark:bg-white/5 rounded-xl p-3">
                <p className="text-[9px] font-bold uppercase tracking-wide text-neutral-400 dark:text-white/35 mb-1">Prima oficina</p>
              ...
              <div className="bg-neutral-50 dark:bg-white/5 rounded-xl p-3">
                ...
                <p className="text-[10px] text-neutral-400 dark:text-white/35 mt-1">ejecutivos</p>
              </div>
            </div>
          )}
          ...
          {ce && ce.en_convencion.length > 0 && (
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 dark:text-white/35 mb-2">Nivel por ejecutivo</p>
              ...
              {ce.cerca.map(v => (
                <div key={v.entity} className="bg-sky-50 dark:bg-sky-950/20 border-l-[3px] border-sky-500 rounded-r-xl p-2.5 mb-2">
                  ...
                  <div className="h-1.5 bg-sky-100 dark:bg-sky-950/30 rounded-full overflow-hidden mt-1.5">
                    <div className="h-full rounded-full" style={{ width: `${v.pct_conv}%`, background: 'linear-gradient(90deg,#0284c7,#38bdf8)' }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {prod?.meta_monto != null && (
            <div>
              ...
              <div className="h-1.5 bg-neutral-200 dark:bg-white/10 rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${Math.min(100, prod.meta_pct ?? 0)}%`, background: 'linear-gradient(90deg,#059669,#34D399)' }} />
              </div>
            </div>
          )}
        </>
...
function SolicitudesPanel({ usuario }: { usuario: Usuario }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const ids = users.map(u => u.id);
      const nameMap = new Map(users.map(u => [u.id, `${u.nombre} ${u.apellidos}`]));
  ...
  return (
    <Panel
      stripe="bg-gradient-to-r from-[#7C3AED] to-[#A78BFA]"
      eyebrow="De tu equipo"
      title="Solicitudes"
      desc="Trámites abiertos de la oficina"
    >
      {loading ? (
        ...
        <>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-neutral-400 dark:text-white/35">Bandeja completa de la oficina</p>
            </div>
...
export function GerenteSections({ usuario }: { usuario: Usuario }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-lg font-extrabold text-neutral-900 dark:text-white leading-tight">Gerente</p>
          <p className="text-[11px] text-neutral-400 dark:text-white/35">Vista de tu sucursal</p>
        </div>
      </div>
