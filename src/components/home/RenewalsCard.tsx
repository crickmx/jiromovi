import { RefreshCw, ChevronRight, Calendar } from 'lucide-react';
import { cn } from '@/lib/utils';

interface Renewal {
  id: string;
  id_docto: string;
  poliza: string;
  cliente: string;
  compania: string;
  ramo: string;
  prima_neta: number;
  vigencia_hasta: string;
  dias_restantes: number;
  urgencia: 'critico' | 'urgente' | 'proximo' | 'normal';
}

interface Props {
  data: Renewal[] | null;
  loading: boolean;
  onViewMore?: () => void;
  onClickItem?: (item: Renewal) => void;
}

const urgenciaConfig = {
  critico: {
    bg: 'bg-red-50 dark:bg-red-500/10',
    text: 'text-red-700 dark:text-red-400',
    border: 'border-red-200/60 dark:border-red-500/20',
    dot: 'bg-red-500',
    label: 'Vence hoy',
  },
  urgente: {
    bg: 'bg-amber-50 dark:bg-amber-500/10',
    text: 'text-amber-700 dark:text-amber-400',
    border: 'border-amber-200/60 dark:border-amber-500/20',
    dot: 'bg-amber-500',
    label: 'Urgente',
  },
  proximo: {
    bg: 'bg-sky-50 dark:bg-sky-500/10',
    text: 'text-sky-700 dark:text-sky-400',
    border: 'border-sky-200/60 dark:border-sky-500/20',
    dot: 'bg-sky-500',
    label: 'Proximo',
  },
  normal: {
    bg: 'bg-neutral-50 dark:bg-white/5',
    text: 'text-neutral-600 dark:text-white/50',
    border: 'border-neutral-200/60 dark:border-white/8',
    dot: 'bg-neutral-400',
    label: '',
  },
};

function formatPrima(value: number): string {
  if (value >= 1_000_000) return `$${(value / 1_000_000).toFixed(1)}M`;
  if (value >= 1_000) return `$${(value / 1_000).toFixed(0)}K`;
  return `$${value.toFixed(0)}`;
}

export function RenewalsCard({ data, loading, onViewMore, onClickItem }: Props) {
  if (loading) {
    return (
      <div className="bg-white dark:bg-neutral-800/50 rounded-2xl border border-neutral-200/60 dark:border-white/8 shadow-card p-5 animate-pulse">
        <div className="flex items-center gap-2.5 mb-4">
          <div className="w-9 h-9 rounded-xl bg-neutral-100 dark:bg-white/8" />
          <div className="h-4 w-36 bg-neutral-100 dark:bg-white/8 rounded" />
        </div>
        <div className="space-y-3">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-14 bg-neutral-100 dark:bg-white/8 rounded-xl" />
          ))}
        </div>
      </div>
    );
  }

  const renewals = data || [];

  return (
    <div className="bg-white dark:bg-neutral-800/50 rounded-2xl border border-neutral-200/60 dark:border-white/8 shadow-card p-5 h-full flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-500/15">
            <RefreshCw className="w-4.5 h-4.5 text-amber-600 dark:text-amber-400" />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-neutral-900 dark:text-white">Proximas Renovaciones</h3>
            <p className="text-[11px] text-neutral-400 dark:text-white/30">Proximos 90 dias</p>
          </div>
        </div>
        {renewals.length > 0 && (
          <span className="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 rounded-md">
            {renewals.filter(r => r.urgencia === 'critico').length > 0
              ? `${renewals.filter(r => r.urgencia === 'critico').length} hoy`
              : `${renewals.length}`}
          </span>
        )}
      </div>

      {/* List */}
      <div className="flex-1 space-y-2">
        {renewals.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Calendar className="w-8 h-8 text-neutral-200 dark:text-white/15 mb-2" />
            <p className="text-sm text-neutral-400 dark:text-white/30">Sin renovaciones proximas</p>
          </div>
        ) : (
          renewals.map((renewal) => {
            const config = urgenciaConfig[renewal.urgencia];
            return (
              <button
                key={renewal.id}
                onClick={() => onClickItem?.(renewal)}
                className={cn(
                  "w-full flex items-center gap-3 p-2.5 rounded-xl border transition-all duration-150 text-left group",
                  config.border,
                  "hover:bg-neutral-50 dark:hover:bg-white/5 active:scale-[0.98]"
                )}
              >
                {/* Urgency indicator */}
                <div className="flex-shrink-0">
                  <div className={cn("w-2 h-2 rounded-full", config.dot)} />
                </div>

                {/* Content */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-xs font-semibold text-neutral-900 dark:text-white truncate">
                      {renewal.cliente}
                    </p>
                  </div>
                  <p className="text-[10px] text-neutral-500 dark:text-white/40 truncate mt-0.5">
                    {renewal.poliza} - {renewal.compania.split(' ')[0]}
                  </p>
                </div>

                {/* Right side */}
                <div className="flex-shrink-0 text-right">
                  <div className={cn("text-[11px] font-bold", config.text)}>
                    {renewal.dias_restantes === 0 ? 'Hoy' : `${renewal.dias_restantes}d`}
                  </div>
                  <p className="text-[10px] text-neutral-400 dark:text-white/25 mt-0.5">
                    {formatPrima(renewal.prima_neta)}
                  </p>
                </div>
              </button>
            );
          })
        )}
      </div>

      {/* Footer */}
      {onViewMore && (
        <button
          onClick={onViewMore}
          className="mt-3 w-full flex items-center justify-center gap-1.5 py-2 rounded-xl bg-neutral-50 dark:bg-white/5 hover:bg-neutral-100 dark:hover:bg-white/8 text-xs font-semibold text-neutral-600 dark:text-white/50 transition-colors group"
        >
          Ver todas las renovaciones
          <ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
        </button>
      )}
    </div>
  );
}
