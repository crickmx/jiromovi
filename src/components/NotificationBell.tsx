...
export function NotificationBell({ compact, dropdownSide = 'right', fixedPanel }: NotificationBellProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (fixedPanel && !isMobile) calculateFixedPosition();
    }

    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [isOpen, fixedPanel, isMobile, calculateFixedPosition]);
  ...
  const getModuleIcon = (modulo: string) => moduleIcons[modulo] || Bell;
  ...
  // ── Panel content (shared between mobile sheet and desktop dropdown) ─────────
  const panelContent = (
    <>
      {/* Header */}
      <div className="p-4 border-b border-neutral-200 dark:border-white/10 flex-shrink-0">
        {/* Mobile drag handle */}
        {isMobile && (
        ...
        {modules.length > 0 && (
          <div className="mt-3 relative">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 dark:text-neutral-500" />
            <select
              value={filterModule}
            ...
            {filteredNotifications.map((notification) => {
              ...
              return (
                <div
                  key={notification.id}
                  className={cn(
                    'p-4 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors cursor-pointer',
      ...
      {(notifications || []).length > 0 && (
        <div className="p-3 border-t border-neutral-200 dark:border-white/10 bg-neutral-50 dark:bg-white/5 flex-shrink-0">
    ...
    <div className="relative" ref={containerRef}>
      ...
      {isOpen && isMobile && (
        <>
          ...
          <div
            ref={panelRef}
            className="fixed inset-x-0 bottom-0 z-[9999] flex flex-col bg-white dark:bg-neutral-900 rounded-t-2xl shadow-2xl border-t border-neutral-200 dark:border-white/10 animate-in slide-in-from-bottom duration-300"
