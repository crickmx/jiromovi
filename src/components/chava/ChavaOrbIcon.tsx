...
export function ChavaOrbIcon({ size = 'md', animate = false, className, sidebarVariant = false }: ChavaOrbIconProps) {
  const px = SIZE_PX[size];
  const uid = `co-${size}${sidebarVariant ? '-sb' : ''}`;

  const c = sidebarVariant ? {
    arc1: 'rgba(255,255,255,0)',
    arc2: 'rgba(255,255,255,0)',
    arc3: 'rgba(255,255,255,0)',
    arc1end: 'rgba(255,255,255,1)',
    arc2end: 'rgba(255,255,255,1)',
    arc3end: 'rgba(255,255,255,1)',
    track: 'rgba(255,255,255,0.22)',
    dot1: '#ffffff', dot2: '#ffffff', dot3: '#ffffff',
    coreFill: '#ffffff', coreInner: 'rgba(200,220,255,0.8)',
  } : {
    arc1: '#38bdf8', arc2: '#7dd3fc', arc3: '#bae6fd',
    arc1end: '#38bdf8', arc2end: '#7dd3fc', arc3end: '#bae6fd',
    track: '#1e4a6e',
  ...
  return (
    <div className={cn('relative inline-flex items-center justify-center flex-shrink-0', SIZE_CLASS[size], className)}>
      ...
      >
        ...
        {/* Background circle — only in default variant */}
        {!sidebarVariant && (
          <>
        ...
        {/* Core */}
        <circle
          cx="70" cy="70" r="8" fill={c.coreFill}
          filter={`url(#${uid}-glow-strong)`}
          style={{ animation: `${uid}-core 2s ease-in-out infinite` }}
        />
        <circle cx="70" cy="70" r="4" fill={c.coreInner} />

        {/* Sparkle dots */}
        <circle cx="70" cy="70" r="1.5" fill={c.dot2} opacity="0.9" style={{ animation: `${uid}-spark 1.8s ease-in-out infinite`, transformOrigin: '70px 70px' }} />
        <circle cx="70" cy="70" r="1.5" fill={c.dot1} opacity="0.7" style={{ animation: `${uid}-spark 2.4s ease-in-out infinite 0.6s`, transformOrigin: '70px 70px' }} />
        <circle cx="70" cy="70" r="1.5" fill={c.dot3} opacity="0.8" style={{ animation: `${uid}-spark 1.6s ease-in-out infinite 1.2s`, transformOrigin: '70px 70px' }} />
        ...
        <style>{`
          ...
          @keyframes ${uid}-spark {
            ...
            30% { opacity: 1; transform: translate(22px,-18px) scale(1.5); }
            60% { opacity: 0.5; transform: translate(-28px,12px) scale(0.8); }
          }
