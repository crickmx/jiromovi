...
import { Ticket, Plus, Search, Filter, AlertCircle, Clock, CheckCircle2, XCircle, FileText } from 'lucide-react';
...
export function Tickets() {
  ...
  const loadData = async () => {
    setLoading(true);
    await Promise.all([loadEstatus(), loadTickets()]);
    setLoading(false);
  };
  ...
  const filteredTickets = tickets.filter(ticket => {
    const matchSearch = ticket.folio.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       ticket.instrucciones.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       ticket.poliza?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       ticket.agente?.nombre_completo.toLowerCase().includes(searchTerm.toLowerCase());

    const matchEstatus = selectedEstatus === 'todos' || ticket.estatus?.id === selectedEstatus;
    const matchPrioridad = selectedPrioridad === 'todas' || ticket.prioridad === selectedPrioridad;
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-soft border border-neutral-200 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-neutral-700 mb-2">
            No hay tickets {activeTab === 'cerrados' ? 'cerrados' : 'activos'}
          </h3>
          <p className="text-neutral-500">
            {activeTab === 'activos' ? 'Crea tu primer ticket para comenzar' : 'No tienes tickets cerrados'}
          </p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    <span className="text-lg font-bold text-primary-600">{ticket.folio}</span>
                    {ticket.estatus && (
                      <span
                    ...
                    <span className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-semibold border ${getPrioridadColor(ticket.prioridad)}`}>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-neutral-600">
                    {ticket.agente && (
                      <span className="flex items-center space-x-1">
                        ...
                        <span>{ticket.agente.nombre_completo}</span>
                      </span>
                    )}
                    {ticket.poliza && (
                      <span className="flex items-center space-x-1">
                    ...
                    {ticket.ticket_asignaciones.length > 0 && (
                      <span className="flex items-center space-x-1">
                        ...
                        <span>
                          {ticket.ticket_asignaciones.map(a => a.ejecutivo?.nombre_completo).join(', ')}
                        </span>
                      </span>
                    )}
                  </div>
                ...
                <div className="text-right text-sm text-neutral-500 ml-4">
                  ...
                  <div className="text-xs mt-1">
                    {new Date(ticket.fecha_creacion).toLocaleTimeString('es-MX', {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}
                  </div>
