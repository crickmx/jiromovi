...
export function SeguwalletDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (clients.length > 0) {
        // sicas_client_id stores the cliente name (from sicas_documents.cliente)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <button onClick={() => navigate('/seguwallet/polizas')} className="bg-white rounded-2xl border border-neutral-200/50 p-5 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group text-left">
        ...
        <button onClick={() => navigate('/seguwallet/polizas')} className={cn("bg-white rounded-2xl border border-neutral-200/50 p-5 shadow-sm hover:shadow-md transition-all group text-left", expiringCount > 0 ? "hover:border-amber-200" : "hover:border-blue-200")}>
          <div className={cn("p-2.5 rounded-xl w-fit mb-3 transition-colors", expiringCount > 0 ? "bg-amber-50 group-hover:bg-amber-100" : "bg-neutral-50 group-hover:bg-neutral-100")}>
            {expiringCount > 0 ? <AlertTriangle className="w-5 h-5 text-amber-600" /> : <Clock className="w-5 h-5 text-neutral-400" />}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-200/50 shadow-sm p-5">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              {agentInfo.oficinas?.nombre && <p className="text-xs text-neutral-500 mt-0.5">{agentInfo.oficinas.nombre}</p>}
            </div>
            {agentInfo.celular_laboral && (
              <a href={`tel:${agentInfo.celular_laboral}`} className="px-4 py-2 rounded-xl bg-blue-50 text-[#1C37E0] text-xs font-semibold hover:bg-blue-100 transition-colors">
                Contactar
              </a>
            )}
          </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <button onClick={() => navigate('/seguwallet/polizas')} className="bg-[#1C37E0] rounded-2xl p-5 text-white shadow-lg shadow-blue-600/20 hover:bg-[#1630C8] hover:shadow-xl transition-all text-left">
