...
1. **Editor de Sidebar en `MobileDrawer.tsx`**: el drag&drop/grupos/badges del Editor de Sidebar solo aplican a `PrimarySidebar.tsx`/`SecondarySidebar.tsx` (desktop + su propio `mobileMode`). `MobileDrawer.tsx` (menú hamburguesa) tiene su propia copia independiente de la lógica de nav (`NAV_ORDER`/`workspace.items` directo) y no hereda nada de esto. Ricardo confirmó que sí lo quiere igual — retomar extendiendo `MobileDrawer.tsx` para usar `useSidebarConfig()`/`useSidebarItemsConfig()` igual que los otros dos.
...
4. **Marcar trámites como "Requiere atención" manualmente**: hoy el Kanban solo mueve un trámite a esa columna por lógica automática (quién actuó último). Ricardo quiere poder marcarlo a mano también.
5. **Reacciones a comentarios de un trámite**: agregar la opción de reaccionar (tipo emoji) a un comentario en `TramiteComentarios.tsx`.
6. **Bug a investigar: comentario nuevo en un trámite donde Ricardo participa no generó notificación ni movió el Kanban a "Requiere atención"**. Revisar el flujo de creación de comentario (`TramiteComentarios.tsx`) — ¿dispara `crearNotificacion` para los participantes? ¿actualiza `ultima_accion_por`/el campo que usa el Kanban para clasificar "Requiere atención"? Probablemente el comentario no toca esas columnas en absoluto — confirmar antes de asumir.
