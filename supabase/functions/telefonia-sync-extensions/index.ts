...
async function pbxLogin(user: string, encodedPass: string): Promise<string> {
  const { bodyText } = await pbxRequest("/api/v1.0/login", {
    method: "POST",
    body: JSON.stringify({ username: user, password: encodedPass, language: "es", supportcrx: true }),
  });
  const data = JSON.parse(bodyText);
  if (data.errcode !== 0) throw new Error(`PBX login failed: ${JSON.stringify(data)}`);
  // Session cookie set by server; we don't need to track it separately for GET requests
  // but we need the websession token from the cookie header — re-read with cookie tracking
  return data.websession ?? "";
}
...
async function pbxLoginWithCookie(user: string, encodedPass: string): Promise<string> {
  ...
  const packet = buildRequest(method, "/api/v1.0/login", bodyStr, []);
  const tcp = await Deno.connect({ hostname: PBX_HOST, port: PBX_PORT });
  const tls = await Deno.startTls(tcp, { hostname: "www.UCCPBX.com", caCerts: [UCCPBX_CA] });
  try {
    let sent = 0;
...
// Normalize name for fuzzy matching: uppercase, strip accents, collapse spaces
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Fetch ALL extensions via the correct endpoint ──────────────────────────
    // Discovered from web UI: /api/v1.0/extension/searchsummary
    ...
    // ── Filter: only 3-4 digit numeric extensions, skip IVR/queue ────────────
    ...
    for (const ext of pbxExts) {
      // Skip if already assigned in MOVI
      const alreadyOwned = users.find(u =>
        u.extension_telefonica?.trim() === ext.number
      );
      if (alreadyOwned) {
      ...
      // Try to find a MOVI user without an extension, matching by email then name
      ...
      if (ext.email) {
        const found = users.find(u =>
          !u.extension_telefonica?.trim() &&
          u.email?.toLowerCase() === ext.email
        );
        if (found) { matched = found; method = "email"; }
      }

      // 2. Full nombre_completo match
      if (!matched && ext.name) {
      ...
      // 3. Partial match: PBX name is a sub-part of MOVI nombre_completo
      if (!matched && ext.name) {
        const nPbx = norm(ext.name);
        const found = users.find(u => {
          if (u.extension_telefonica?.trim()) return false;
      ...
      if (!matched) {
        unmatched.push({ ext: ext.number, pbx_name: ext.name });
        continue;
      }

      if (!dryRun) {
        await db.from("usuarios")
          .update({ extension_telefonica: ext.number })
          .eq("id", matched.id);
