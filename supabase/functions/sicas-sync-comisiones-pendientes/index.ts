...
async function consultarComisionesPendientes(
  endpoint: string,
  username: string,
  password: string,
  dateFrom?: string,
  ...
  console.log(`[SICAS] Consultando comisiones pendientes - Página ${page}`);

  const reportCode = 'H03492_ALL';
  const infoSort = 'DatDocumentos.Documento DESC';

  // Construir ConditionsAdd para filtro de fechas
  let conditionsAdd = '';
  if (dateFrom && dateTo) {
    // Formato: Campo|Operador|Valor
    // Para rango de fechas: Campo BETWEEN dateFrom AND dateTo
    conditionsAdd = `DatDocumentos.FEmision|>=|${dateFrom}|AND|DatDocumentos.FEmision|<=|${dateTo}`;
  }
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  try {
    ...
    if (conditionsAdd) {
      console.log(`[SICAS] Filtro de fechas: ${dateFrom} a ${dateTo}`);
    }
  ...
  // Detectar error interno de SICAS
  ...
  // Parsear comisiones
  const comisiones: ComisionPendiente[] = [];
  ...
  console.log(`[SICAS] Parseadas ${comisiones.length} comisiones pendientes`);
...
async function guardarComisionesCache(
  ...
  console.log(`[Cache] Guardando ${comisiones.length} comisiones...`);
...
async function registrarSyncLog(
  ...
  await supabase
    ...
    .insert({
      report_code: 'H03492_ALL',
      report_name: 'Comisiones Pendientes',
      status,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[Sync] STAGE: FETCH_SICAS - Consultando comisiones pendientes...');

    const allComisiones: ComisionPendiente[] = [];
    let currentPage = 1;
    ...
    while (currentPage <= maxPages) {
      try {
        ...
        const result = await consultarComisionesPendientes(
          sicasUrl,
          sicasUsuario,
          sicasPassword,
          dateFrom,
    ...
    await registrarSyncLog(
      ...
      dateFrom,
      dateTo,
      !hasRealData ? 'No se obtuvieron comisiones de SICAS' : undefined,
      lastSicasDetails,
      lastDiagnostic,
      lastRequestInfo
    );
