...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[process-excel-staging] Processing file:', fileName, 'Size:', file.size);
    ...
    console.log(`[process-excel-staging] Loaded ${usuarios?.length || 0} users, ${mappingsMap.size} persistent mappings`);

    const vendorNamesRaw = jsonData
      .map((row: any) => row[detectedColumns.vendNombre]?.toString()?.trim() || '')
      .filter(name => name.length > 0);

    const uniqueVendorNames = Array.from(new Set(vendorNamesRaw));

    const normalizedNamesMap = new Map<string, string>();
    for (const rawName of uniqueVendorNames) {
      const { data: normalizedName } = await supabase.rpc('normalize_person_name', {
        name_input: rawName
      });
      if (normalizedName) {
        normalizedNamesMap.set(rawName, normalizedName);
      }
    }

    console.log(`[process-excel-staging] Normalized ${normalizedNamesMap.size} unique vendor names`);
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (vendorNameRaw && status !== 'discard') {
        vendorNameNorm = normalizedNamesMap.get(vendorNameRaw) || '';
      ...
      if (status === 'valid') {
        const item = {
          ...
          prima_neta: parseNumberMx(row[headersNormalizedMap['primaneta']]) || importeValue || 0,
          importe_base: importeValue || 0,
