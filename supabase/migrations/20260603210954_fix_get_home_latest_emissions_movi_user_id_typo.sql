/*
  ...
  ## Problem
  The function `get_home_latest_emissions` had a typo in the JOIN condition for
  Agente-level users: `m.mowi_user_id` instead of `m.movi_user_id`. This caused
  the JOIN to always return 0 rows for agents, so their "latest emissions" section
  on the dashboard was always empty.

  ## Change
  - Line in JOIN: `m.mowi_user_id = p_user_id` → `m.movi_user_id = p_user_id`
*/
...
BEGIN
  ...
  ELSE
    -- Agente: use vendor mapping (fixed typo: mowi → movi)
    SELECT COALESCE(jsonb_agg(row_to_json(r)), '[]'::jsonb)
    INTO result
    FROM (
