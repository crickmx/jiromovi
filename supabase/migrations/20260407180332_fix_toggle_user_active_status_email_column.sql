/*
  # Fix Toggle User Active Status Function - Correct Email Column
  
  1. Changes
    - Replace `email` with `email_laboral` throughout the function
    - This fixes the "column email does not exist" error
  
  2. No Security Changes
    - All security logic remains the same
    - Only fixing column name reference
*/
...
BEGIN
  ...
  -- Get target user info (using email_laboral instead of email)
