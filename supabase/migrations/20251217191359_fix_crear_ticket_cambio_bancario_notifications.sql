/*
  # Corregir función crear_ticket_cambio_bancario para usar tabla notificaciones correcta

  1. Cambios
    - Actualizar función crear_ticket_cambio_bancario para usar tabla `notificaciones` en lugar de `notificaciones_globales`
    - La tabla `notificaciones` tiene la columna `usuario_id` correcta
    - Ajustar nombres de columnas: `enlace` -> `url`, `fecha_hora` -> `created_at`

  2. Notas
    - Corrige el error: column "usuario_id" of relation "notificaciones_globales" does not exist
*/

-- Recrear función con tabla correcta
...
BEGIN
  ...
  ELSE
    ...
    -- Crear notificación para el usuario usando tabla correcta
...
COMMENT ON FUNCTION crear_ticket_cambio_bancario IS 'Crea o actualiza automáticamente un ticket de cambios bancarios cuando el usuario modifica su información de pago. Evita duplicados verificando tickets recientes (últimos 15 minutos). Corregida para usar tabla notificaciones correcta.';