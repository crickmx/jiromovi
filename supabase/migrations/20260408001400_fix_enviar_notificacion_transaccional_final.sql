/*
  # Arreglar definitivamente enviar_notificacion_transaccional
  
  Corregir la función para usar whatsapp_plantilla en lugar de whatsapp_template
  y determinar canales activos correctamente desde las columnas enviar_*
*/
    ...
    CASE
      WHEN p.enviar_correo OR p.enviar_whatsapp OR p.enviar_notificacion THEN
        ARRAY(
          ...
          WHERE (unnest(ARRAY['email', 'whatsapp', 'in_app']) = 'email' AND COALESCE(p.enviar_correo, true))
             OR (unnest(ARRAY['email', 'whatsapp', 'in_app']) = 'whatsapp' AND COALESCE(p.enviar_whatsapp, true))
             OR (unnest(ARRAY['email', 'whatsapp', 'in_app']) = 'in_app' AND COALESCE(p.enviar_notificacion, true))
        )
...
COMMENT ON FUNCTION enviar_notificacion_transaccional IS 
  'Envía notificaciones transaccionales con soporte para adjuntos. Usa whatsapp_plantilla y detecta canales desde enviar_correo/whatsapp/notificacion.';
