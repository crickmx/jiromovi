/*
  # Fix: Normalizar régimen fiscal en calculate_batch_fiscal_aggregates

  1. Problema
     - Necesita normalización robusta del régimen fiscal
     - Manejo de variantes y valores NULL

  2. Solución
     - Normalización consistente con el trigger de details
     - Asegurar que siempre se use un régimen válido
*/
...
BEGIN
  ...
  -- 2. Obtener régimen fiscal ACTUAL del usuario y normalizar
  ...
  IF v_regimen_fiscal = 'HONORARIOS' THEN
    ...
    v_tax_version := 'HONORARIOS_V8_NORMALIZED';
  ...
  ELSIF v_regimen_fiscal = 'RESICO' THEN
    ...
    v_tax_version := 'RESICO_V8_NORMALIZED';
  ...
  ELSIF v_regimen_fiscal = 'ASIMILADOS' THEN
    ...
    v_tax_version := 'ASIMILADOS_V8_NORMALIZED';
...
'V8: Normalización robusta del régimen fiscal. ISR sobre (Total + IVA), Ret IVA = IVA × 2/3.
