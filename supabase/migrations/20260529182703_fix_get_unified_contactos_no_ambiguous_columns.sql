/*
  # Fix get_unified_contactos - eliminate all ambiguous column references

  Complete rewrite using fully-prefixed aliases in both CTEs so that
  PL/pgSQL never confuses CTE column names with OUT parameter names.
*/
...
BEGIN
  ...
  WITH crm_with_sw AS (
    SELECT
      ...
      sw.last_login                                             AS cws_sw_last_login,
  ...
  sw_only AS (
    SELECT
      ...
      sw2.last_login                                            AS so_sw_last_login,
  ...
  ORDER BY 11 DESC  -- fecha_creacion DESC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
