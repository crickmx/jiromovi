/*
  # Corregir Seguridad de Función Pública

  1. Cambios
    - Cambiar de SECURITY INVOKER a SECURITY DEFINER
    - Esto permite que usuarios anónimos obtengan datos públicos
    - La función se ejecuta con permisos del owner, no del caller
  
  2. Seguridad
    - Función valida que is_published = true
    - Solo devuelve datos de usuarios activos
...
-- Comentario
COMMENT ON FUNCTION get_public_web_page_by_slug IS 'Obtiene los datos de la página pública de un asesor por su slug. Acceso público sin autenticación. SECURITY DEFINER para bypass de RLS.';
