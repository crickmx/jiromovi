/*
  # Corrección CRÍTICA: Fórmulas ASIMILADOS según CSV Oficial

  ## Problema
  Las fórmulas de ISR para ASIMILADOS estaban incorrectas:
  
  ❌ ANTES (Incorrecto):
  - ISR Vida = (Vida - (RetContable / 1.09)) × 10%
  - ISR Daños = (SinVida - (Dispersión / 1.09)) × 10%
  ...
  ## Diferencia
  Método anterior restaba las retenciones antes de calcular ISR.
  Método correcto simplemente divide por 1.16 (Vida) o 1.09 (SinVida).

  ## Ejemplo Validado (CSV Oficial)
  - Vida: $544.20
  - Sin Vida: $14,263.87
  - ISR Vida = (544.20 / 1.16) × 0.10 = $46.91 ✓
  - ISR Daños = (14,263.87 / 1.09) × 0.10 = $1,308.43 ✓
  - ISR Total = $1,355.53 ✓ (antes: $1,355.07 ✗)
  - Total Final = $12,081.72 ✓
*/

-- Reemplazar función con fórmulas correctas del CSV
...
BEGIN
  ...
  IF v_regimen_fiscal = 'HONORARIOS' THEN
    ...
    v_tax_version := 'HONORARIOS_CSV_V4';
  ...
  ELSIF v_regimen_fiscal = 'RESICO' THEN
    ...
    -- Ret ISR = Comisión Base Total × 1.25%
  ...
  ELSIF v_regimen_fiscal = 'ASIMILADOS' THEN
    ...
    -- 3. ISR VIDA = (Vida / 1.16) × 0.10
    --    CSV: =(544/1.16)*.10 = $46.91
    v_isr_vida_temp := (v_commission_vida / 1.16) * 0.10;

    -- 4. ISR DAÑOS = (SinVida / 1.09) × 0.10
    --    CSV: =((14808.07-544.20)/1.09)*0.1 = $1,308.43
    ...
    v_tax_version := 'ASIMILADOS_CSV_V4';
...
COMMENT ON FUNCTION calculate_batch_fiscal_aggregates IS 'Fórmulas fiscales validadas contra CSV oficial. ASIMILADOS: ISR Vida = Vida/1.16, ISR Daños = SinVida/1.09';
