/*
  # Corregir nombre de columna en función enviar_notificacion_global

  1. Cambios
    - Reemplazar telefono_movil por celular_personal
    - Usar celular_personal como columna principal
    - Si no hay celular_personal, intentar con celular_laboral

  2. Notas
    - Corrige error "column telefono_movil does not exist"
    - Mantiene toda la lógica de envío de WhatsApp
*/

-- Eliminar función anterior
DROP FUNCTION IF EXISTS enviar_notificacion_global(text, text, text, jsonb, uuid, boolean);

-- Recrear función con nombre de columna correcto
...
BEGIN
  ...
  IF (p_destinatarios->>'tipo') = 'todos' THEN
    ...
    FOR v_user_record IN 
      SELECT id, nombre, apellidos, celular_personal, celular_laboral 
      FROM usuarios 
      WHERE estado = 'activo'
    LOOP
      ...
      -- Priorizar celular_personal, si no existe usar celular_laboral
      v_telefono := COALESCE(NULLIF(v_user_record.celular_personal, ''), NULLIF(v_user_record.celular_laboral, ''));
      
      IF p_enviar_whatsapp AND v_telefono IS NOT NULL THEN
        BEGIN
