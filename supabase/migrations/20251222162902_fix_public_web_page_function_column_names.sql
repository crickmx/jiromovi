/*
  # Corregir nombres de columnas en función get_public_web_page_by_slug

  1. Problema
    - La función usa `foto_url` pero la columna es `imagen_perfil_url`
    - Esto causaba un error y las páginas públicas no se mostraban
  
  2. Solución
    - Actualizar la función para usar los nombres correctos de columna
    - `u.foto_url` -> `u.imagen_perfil_url`
*/
