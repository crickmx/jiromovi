/*
  # Update SICAS sync diagnostics with vendor overlap metrics

  1. Changes
    - Drops and recreates `get_sicas_sync_diagnostics` function
    - Adds vendor overlap analysis: how many document vendor IDs are mapped vs unmapped
    - Adds stuck sync runs count
    - Adds admin RLS simulation count
    - Helps admins identify data gaps in vendor-to-user mappings

  2. Security
    - SECURITY DEFINER to bypass RLS for accurate counts
...
BEGIN
  ...
  WHERE sd.vend_id IS NOT NULL AND sd.vend_id != ''
    AND (
      EXISTS (SELECT 1 FROM sicas_mapeo_vendedor_usuario m WHERE m.id_vendedor::text = sd.vend_id)
  ...
  FROM sicas_documents sd
  WHERE sd.vend_id IS NOT NULL AND sd.vend_id != ''
    AND NOT EXISTS (SELECT 1 FROM sicas_mapeo_vendedor_usuario m WHERE m.id_vendedor::text = sd.vend_id)
