/*
  # Corrección Fiscal SOLO para HONORARIOS y RESICO

  REGLA ABSOLUTA: ASIMILADOS ES INTOCABLE

  1. Función SQL para calcular fiscal
    - Guard clause obligatorio: Si régimen es ASIMILADOS → return sin hacer nada
    - Solo procesa HONORARIOS y RESICO
    - Calcula y persiste en commission_batches
  ...
  3. Tax Versions
    ...
    - ASIMILADOS → NO TOCAR, mantener su versión actual si existe
*/

-- Función para calcular fiscal agregado del lote (SOLO HONORARIOS Y RESICO)
...
BEGIN
  ...
  -- 3. Calcular totales por ramo
  FOR v_detail IN
    SELECT
      LOWER(COALESCE(ramo, '')) as ramo_lower,
      COALESCE(commission_bruta, 0) as comision
    FROM commission_details
    WHERE batch_id = p_batch_id
      AND commission_bruta IS NOT NULL
  LOOP
...
COMMENT ON FUNCTION calculate_batch_fiscal_aggregates IS 'Calcula agregados fiscales SOLO para HONORARIOS y RESICO. ASIMILADOS se salta completamente (guard clause).';
