/*
  # Actualizar función para enviar notificaciones por WhatsApp

  1. Cambios en la función
    - Agregar parámetro `p_enviar_whatsapp` (boolean)
    - Guardar el valor en la tabla `notificaciones_globales`
    - Enviar mensajes por WhatsApp a usuarios con teléfono cuando esté activo
    - Actualizar contadores de envíos exitosos/fallidos

  2. Notas
    - Si p_enviar_whatsapp es true, enviará por WhatsApp a todos los destinatarios con teléfono
    - Los envíos de WhatsApp se hacen de forma asíncrona
    - Se registran los totales de envíos en la tabla notificaciones_globales
*/

-- Eliminar la función anterior
DROP FUNCTION IF EXISTS enviar_notificacion_global(text, text, text, jsonb, uuid);

-- Recrear función con soporte de WhatsApp
...
BEGIN
  ...
  IF (p_destinatarios->>'tipo') = 'todos' THEN
    ...
    FOR v_user_record IN 
      SELECT id, nombre, apellidos, telefono_movil FROM usuarios WHERE estado = 'activo'
    LOOP
      ...
      IF p_enviar_whatsapp AND v_user_record.telefono_movil IS NOT NULL THEN
        BEGIN
          PERFORM net.http_post(
            ...
            body := jsonb_build_object(
              ...
              'numero', v_user_record.telefono_movil,
  ...
  ELSIF (p_destinatarios->>'tipo') = 'oficina' THEN
    ...
    FOR v_user_record IN 
      SELECT id, nombre, apellidos, telefono_movil FROM usuarios 
  ...
  ELSIF (p_destinatarios->>'tipo') = 'usuario' THEN
    ...
    SELECT id, nombre, apellidos, telefono_movil INTO v_user_record
    FROM usuarios 
