/*
  # Fix cálculo ISR para HONORARIOS y RESICO

  ## Problema detectado
  La función calculate_batch_fiscal_aggregates calcula el ISR sobre commission_total (líneas 115 y 136)
  cuando debería calcularlo sobre el SUBTOTAL CON IVA (commission_total + IVA)

  ## Fórmulas correctas
  
  ### HONORARIOS:
  - Subtotal con IVA = Commission Total + IVA
  - Retención ISR = (Commission Total + IVA) × 10%
  - Retención IVA = (Sin Vida × 16) / 150
  - Total Neto = Subtotal con IVA - Ret ISR - Ret IVA

  ### RESICO:
  - Subtotal con IVA = Commission Total + IVA
  - Retención ISR = (Commission Total + IVA) × 1.25%
  - Retención IVA = (Sin Vida × 16) / 150
  - Total Neto = Subtotal con IVA - Ret ISR - Ret IVA
*/
...
BEGIN
  ...
  IF v_regimen_fiscal = 'HONORARIOS' THEN
    -- ============================================
    -- HONORARIOS (CORRECCIÓN: ISR sobre total CON IVA)
    -- ============================================
    ...
    -- CORRECCIÓN: ISR se calcula sobre el subtotal CON IVA
    ...
    v_tax_version := 'HONORARIOS_CSV_V6_ISR_CON_IVA';

  ELSIF v_regimen_fiscal = 'RESICO' THEN
    -- ============================================
    -- RESICO (CORRECCIÓN: ISR sobre total CON IVA)
    -- ============================================
    v_retencion_contable := 0;
    v_costo_dispersion := 0;
    ...
    v_tax_version := 'RESICO_CSV_V6_ISR_CON_IVA';
...
'CORRECCIÓN V6: ISR de HONORARIOS (10%) y RESICO (1.25%) se calcula sobre (Commission Total + IVA).
Fórmula correcta: Ret ISR = (Commission Total + IVA) × tasa_isr
ASIMILADOS mantiene su cálculo sin cambios.';
