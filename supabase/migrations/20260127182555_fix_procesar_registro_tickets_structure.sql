/*
  # Corregir función procesar_registro_no_usuario para estructura correcta de tickets

  1. Cambios
    - Actualiza función para usar campos correctos de tabla tickets
    - Campo: assigned_to_user_id en lugar de usuario_id
    - Campo: estatus_id (UUID del estado "Nuevo")
    - Campo: instrucciones en lugar de descripcion
    - Campo: tipo_tramite en lugar de categoria
    - Elimina campos inexistentes: titulo, estado
...
-- Actualizar función para usar estructura correcta de tickets
...
BEGIN
  ...
  IF p_es_agente_jiro THEN
    ...
    FOR v_destinatario IN (
      ...
      -- Crear tarea con estructura correcta
      INSERT INTO tickets (
        tipo_tramite,
        instrucciones,
        prioridad,
...
COMMENT ON FUNCTION procesar_registro_no_usuario IS 'Procesa registro de "Aún no soy usuario", crea tareas (tickets) y envía notificaciones - ACTUALIZADO para usar estructura correcta de tabla tickets';
