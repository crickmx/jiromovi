/*
  # Fix SICAS Dashboard Self-Scope to Use Vendor Mapping

  1. Problem
    - All dashboard functions used `d.usuario_id = p_user_id` for self (agent) scope
    - Only 261 of 101K documents have `usuario_id` populated
    - Agents could not see their production data in the dashboard

  2. Solution
    - Resolve the user's mapped SICAS vendor ID from `sicas_mapeo_vendedor_usuario` or `usuarios.id_sicas`
    - Use `d.vend_id = v_vend_id` instead of `d.usuario_id = p_user_id` for self scope
    - Fallback chain: sicas_mapeo_vendedor_usuario -> usuarios.id_sicas -> usuarios.nombre_sicas (name match)

  3. Functions Updated
    - `get_sicas_user_scope` - now returns vend_id in the scope response
    - `get_sicas_dashboard_kpis` - self scope uses vend_id
    - `get_sicas_dashboard_charts` - self scope uses vend_id
    - `get_sicas_dashboard_top` - self scope uses vend_id
    - `get_sicas_avance_comercial` - self scope uses vend_id

  4. Security
    - Non-admin users still cannot escalate to admin scope
    - Office scope still restricted to user's oficina_id
    - Self scope now correctly restricted to mapped vendor only
    - All functions remain SECURITY DEFINER
*/

-- Helper: resolve user's SICAS vendor ID
CREATE OR REPLACE FUNCTION get_sicas_user_vend_id(p_user_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT mvu.id_sicas_vendedor INTO v_vend_id
  FROM sicas_mapeo_vendedor_usuario mvu
  WHERE mvu.movi_user_id = p_user_id
  LIMIT 1;
  ...
  -- Priority 2: usuarios.id_sicas field
  SELECT u.id_sicas INTO v_vend_id
  FROM usuarios u
  WHERE u.id = p_user_id AND u.id_sicas IS NOT NULL;

  IF v_vend_id IS NOT NULL THEN
    RETURN v_vend_id;
  END IF;

  -- Priority 3: match by nombre_sicas
  SELECT u.nombre_sicas INTO v_nombre_sicas
  FROM usuarios u
  WHERE u.id = p_user_id AND u.nombre_sicas IS NOT NULL;

  IF v_nombre_sicas IS NOT NULL THEN
    SELECT sv.id_sicas INTO v_vend_id
    FROM sicas_vendedores sv
    WHERE UPPER(sv.nombre) = UPPER(v_nombre_sicas)
    LIMIT 1;
  END IF;
...
-- Update get_sicas_user_scope to include vend_id
...
BEGIN
  ...
  -- Resolve vendor ID for self-scope users
...
BEGIN
  ...
  -- Resolve vendor ID for self scope
  IF v_scope = 'self' THEN
    v_vend_id := get_sicas_user_vend_id(p_user_id);
  END IF;
...
-- ============================================================================
-- Fix get_sicas_dashboard_charts
-- ============================================================================
DROP FUNCTION IF EXISTS public.get_sicas_dashboard_charts(uuid, text, uuid, integer, text, date, date);
