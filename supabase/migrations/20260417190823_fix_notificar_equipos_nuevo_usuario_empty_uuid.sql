/*
  # Fix notificar_equipos_nuevo_usuario empty UUID cast error

  The trigger function was doing current_setting('app.current_user_id', true)::uuid
  which fails when the setting is an empty string "".
  Added a NULL check before casting.
*/
...
BEGIN
  ...
  IF v_current_user_id IS NOT NULL AND v_current_user_id <> '' THEN
    ...
    FROM usuarios
    WHERE id = v_current_user_id::uuid;
